...
class TestExcelReaderInit:
    ...
    def test_excel_reader_init_invalid_extension(self):
        ...
        with tempfile.NamedTemporaryFile(suffix='.txt', delete=False) as tmp_file:
            ...
            try:
                with pytest.raises(InvalidFileException, match="Invalid file extension"):
...
class TestDiscoverSheets:
    ...
    def test_discover_sheets_workbook_error(self, mock_load_workbook):
        ...
        with tempfile.NamedTemporaryFile(suffix='.xlsx', delete=False) as tmp_file:
            ...
            try:
                reader = ExcelReader(tmp_file.name)
                with pytest.raises(WorkbookInitializationException, match="Failed to initialize workbook"):
...
class TestExtractSheetData:
    ...
    def test_extract_sheet_data_sheet_not_found(self, mock_load_workbook):
        ...
        with tempfile.NamedTemporaryFile(suffix='.xlsx', delete=False) as tmp_file:
            ...
            try:
                reader = ExcelReader(tmp_file.name)
                with pytest.raises(WorksheetNotFoundException, match="Worksheet 'NonExistent' not found"):
    ...
    def test_extract_sheet_data_worksheet_error(self, mock_load_workbook):
        ...
        with tempfile.NamedTemporaryFile(suffix='.xlsx', delete=False) as tmp_file:
            ...
            try:
                reader = ExcelReader(tmp_file.name)
                with pytest.raises(DataExtractionException, match="Failed to extract data from sheet 'Sheet1'"):
...
class TestMemoryManagement:
    ...
    def test_discover_sheets_with_exceptions(self, mock_load_workbook):
        ...
        with tempfile.NamedTemporaryFile(suffix='.xlsx', delete=False) as tmp_file:
            ...
            try:
                ...
                mock_workbook = Mock()
                mock_workbook.sheetnames = Mock(side_effect=Exception("Sheet access failed"))
                mock_load_workbook.return_value = mock_workbook
    ...
    def test_extract_sheet_data_empty_row_handling(self, mock_load_workbook):
        ...
        mock_rows[0].__iter__ = Mock(return_value=iter(["Header1", "Header2", None, None]))
    ...
    @patch('src.excel_processor.load_workbook')
    def test_validate_sheet_data_integration(self, mock_load_workbook, mock_data_validator_class):
        ...
        with tempfile.NamedTemporaryFile(suffix='.xlsx', delete=False) as tmp_file:
            ...
            try:
                ...
                with patch.object(reader, 'extract_sheet_data', return_value=mock_sheet_data):
